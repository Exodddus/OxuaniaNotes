...
我们可以将[上一章](硬件课程/CAbyQZY/1.md#quantitative-principles-of-computer-design)介绍的处理器执行时间的计算公式用于评估高速缓存的性能，不过需要额外加上**存储器停顿周期数**(memory stall cycles)：
...
- 降低**失效率**：更大的数据块，更大的高速缓存，更高的相联程度（大力出奇迹，一定是看到这个问题的第一反应）
...
- **容量**(capacity)：在程序执行的过程中，高速缓存无法包含所有需要的数据块，于是不得不抛弃一些数据块，但之后可能又需要检索这些数据块，此时失效发生（wxh：做了一桌菜，来了两桌人）
...
>这里涉及到[指令级并行](硬件课程/CAbyQZY/3.md)(ILP)

[**推测**](硬件课程/CAbyQZY/3.md#hardware-based-speculation)(speculation)是高级流水线中的一项重要技术，它基于**分支预测**，在处理器确定指令是否真正需要执行之前就进行尝试性执行。如果推测错误，流水线中的推测指令会被清除，并可能产生不必要的内存引用，从而增加缓存失效率。然而，结合**预取**技术，推测实际上可以降低总体的缓存失效损失。因此，在比较失效率时，需要注意推测执行可能带来的误导性影响。


### Special Instruction Caches

>这里涉及到[数据级并行](硬件课程/CAbyQZY/4.md)(DLP)
...
>这里涉及到[线程级并行](硬件课程/CAbyQZY/5.md)(TLP)
