...
- 于是我们把单词表示成数轴上面的一个轴（one-hot representation），例如`(0,0,1),(0,1,0),(1,0,0)`，对一篇文档的每个词向量相加即可得到每个词的频率向量（TFV），每个文档的向量距离代表其相似度。
    - 问题：
        ...
        - 检索不出同义词，无法捕捉词语之间的语义关系，每个词语都是正交的
          ![image-20250307144926520](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307144926520.png)
        - 耗费空间
    - 逆词频：计算一个词在多少篇文章出现，如果出现得少就更具有代表性
## 2 Distributional similarity based representations

不同人的大脑在听到同一个单词时，被激活的若干个脑区是类似的

![image-20250307150212761](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307150212761.png)

如何了解一个词的意思：通过上下文，如果在某个上下文去掉这个词，再填入的那个词就和原来的词表征就比较相似。

这个过程的两种方法：

![image-20250307150558713](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307150558713.png)

1. continuous bag-of-word (CBOW)
      把上下文送到模型预测`apple`
      ![image-20250307150416126](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307150416126.png)
...
2. skip-gram (SG)
      把`apple`输入进去预测上下文
      ![image-20250307150456076](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307150456076.png)

训练过程宏观考量是调整单词之间的”距离“，因为只有接近才能被激活；这是一个动态平衡的过程（一个单词和别的很多单词都有关系）：

![image-20250307152301563](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307152301563.png)

最后的结果：

![image-20250307153230413](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307153230413.png)

可以在语义空间做计算，例如`king - uncle`这个欧式距离表示从男性到女性的转变：

![image-20250307153345923](NLP_TSL/NLPbyWYR/WordEmbedding.assets/image-20250307153345923.png)
